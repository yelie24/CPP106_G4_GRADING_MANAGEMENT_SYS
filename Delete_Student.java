...
public class Delete_Student extends javax.swing.JInternalFrame {
    //for managing data in the table
    ...
    //to initialize the table with predefined column names
    ...
    // Method to read student data from a file and display it in the table
    private void displayFileInTable() {
        try (BufferedReader reader = new BufferedReader(new FileReader("testingGMS.txt"))) {
            ...
            while ((line = reader.readLine()) != null) {
                String[] data = line.split("\\s+");//Splits the lines based on whitespace
                //ensure there are enough data parts to avoid
                if (data.length >= 22) {
    ...
    private void deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteActionPerformed
        ...
        boolean found = false;//Declare the boolean found to false
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {//loop through each line in the file until the end
                String[] data = line.split("\\s+");
                if (data.length >= 22) { // Check if the line has at least 22 elements to avoid errors
            ...
            if (found) {//Set the condition found if the student number was found and its deleted
